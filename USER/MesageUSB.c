      ...
      if(huart ==&huart1 ){
          ...
          error = HAL_UARTEx_ReceiveToIdle_DMA(&huart1 ,RxDataBuf,Size);
           ...
           error =  HAL_UARTEx_ReceiveToIdle_DMA(&huart1 ,RxDataGSM,ReceivSize);
                ...
                if(huart==&huart2 ){
                error = HAL_UARTEx_ReceiveToIdle_DMA(&huart2 ,RxDataBuf,Size);
                                 if( error == HAL_OK){
                                 ...
                                 // CDC_Transmit_FS(RxDataGPS,SizeGPS);
                                  ...
                                  error = HAL_UARTEx_ReceiveToIdle_DMA(&huart2 ,RxDataBuf,Size);}else{
                               if( error ==HAL_BUSY){ HAL_GPIO_WritePin(GPIOC, LED_WORK_Pin, GPIO_PIN_RESET);}else{
                                if( error ==HAL_ERROR){
                                 MX_USART1_UART_Init();
                 error =  HAL_UARTEx_ReceiveToIdle_DMA(&huart2 ,RxDataGPS,ReceivSize);
                                }}}  }}
                                 statusDMA=0;
}
